        ...
        if data == None:
            ...
            if st.button('Extract Features'):
                ...
                with c1:
                    ...
                    ftmt = ftmt[['pasal']]
                    st.write('Pasal yang menjadi Fitur untuk Prediksi')
                    st.write(ftmt['pasal'].unique())
                with c2:
                    ket = pd.read_csv('verdict_fitur.csv', sep=";")
                    my_expander = st.beta_expander(label='Keterangan dan Contoh')
                    with my_expander:
                    # st.beta_expander('Keterangan dan Contoh Kasus berdasarkan pasal')
                        ...
                        st.table(ket)
            if st.button("Jalankan Prediksi (2)"):
                # model= open("vmodel.pkl", "rb")
                model = open("model_verdict/ver_knn_2.pkl", "rb")
                knn=joblib.load(model)
                dfvalues = pd.DataFrame(list(zip([v1],[v2],[v3],[v4],[v5],[v6],[v7],[v8],[v9],[v10])),columns =['pasal 13', 'pasal 4','pasal 15','pasal 16A', 'pasal 16B', 'pasal 1', 'pasal 5', 'pasal 9', 'pasal 19', 'pasal 17'])
                input_variables = np.array(dfvalues[['pasal 13', 'pasal 4','pasal 15','pasal 16A', 'pasal 16B', 'pasal 1', 'pasal 5', 'pasal 9', 'pasal 19', 'pasal 17']])
            ...
            if st.button("Jalankan Prediksi (3)"):
                # model= open("vmodel.pkl", "rb")
